...
namespace LibUI
{
    public class LoginWindow : Window
    {
            ...
            : base("Login")
        {
            ...
            {
                GlobalVar.username = entryUsername.Text;
                GlobalVar.password = entryPassword.Text;
                ...
                client.Credentials = new NetworkCredential(GlobalVar.username, GlobalVar.password);

                login LoginHandler = (x, y) => int.Parse(client.UploadString(x, y), System.Globalization.NumberStyles.AllowLeadingSign);

                GlobalVar.privilege = LoginHandler(host, command);
                ;

                if (GlobalVar.privilege < 0)
                {
