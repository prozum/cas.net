...
namespace Ast
{
    public class Cos : SysFunc, IInvertable
    {
        ...
        protected override Expression Evaluate(Expression caller)
        {
            ...
            if (res is Integer)
            {
                return ReturnValue(new Irrational((decimal)Math.Cos((res as Integer).value * (degrees ? (Math.PI / 180) : 1)))).Evaluate();
            }

            if (res is Rational)
            {
                return ReturnValue(new Irrational((decimal)Math.Cos((double)(res as Rational).value.value * (degrees ? (Math.PI / 180) : 1)))).Evaluate();
            }

            if (res is Irrational)
            {
                return ReturnValue(new Irrational((decimal)Math.Cos((double)(res as Irrational).value * (degrees ? (Math.PI / 180) : 1)))).Evaluate();
            }
